...
func (client *PortainerAsyncClient) GetEnvironmentStatus(flags ...string) (*PollStatusResponse, error) {
	...
	if doSnapshot {
		...
		case agent.PlatformDocker:
			...
			if err != nil {
				log.Warn().Stack().Err(err).Msg("could not create the Docker snapshot")
			}
			...
			if client.lastSnapshot.Docker != nil {
				...
				if err == nil {
					...
					log.Warn().Stack().Err(err).Msg("could not generate the Docker snapshot patch")
				}
			}
			...
			for _, stack := range client.stackLogCollectionQueue {
				...
				if err != nil {
					log.Warn().Stack().Err(err).Msg("could not retrieve containers for stack")
		...
		case agent.PlatformKubernetes:
			...
			if err != nil {
				log.Warn().Stack().Err(err).Msg("could not create the Kubernetes snapshot")
			}
			...
			if client.lastSnapshot.Kubernetes != nil {
				...
				if err == nil {
					...
					log.Warn().Stack().Err(err).Msg("could not generate the Kubernetes snapshot patch")
				}
			}
		}
