...
func ldapURL(url string) bool {
	...
	if err != nil {
		log.Warn().Str("url", url).Stack().Err(err).Msg("error parsing URL")
...
func (service *Service) certIsRevokedCRL(cert *x509.Certificate, url string) (revoked bool, err error) {
	...
	if shouldFetchCRL {
		...
		if err != nil {
			log.Warn().Str("url", url).Stack().Err(err).Msg("failed fetching CRL")
		...
		if issuer != nil {
			...
			if err != nil {
				log.Warn().Str("url", url).Stack().Err(err).Msg("failed verifying CRL")
...
func parseCertificatePEM(certPEM []byte) (*x509.Certificate, error) {
	...
	if err != nil {
		...
		log.Debug().Stack().Err(err).Msg("certificate parsing error")
